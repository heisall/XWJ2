...
@property NSString *adds1;
@property NSString *adds2;
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    self.con.buyerLabel.text = [NSString stringWithFormat: @"%@ %@",self.name.text,self.phone.text];
    self.con.addressLabel.text = [NSString stringWithFormat:@"%@ %@",self.adds1,self.detailTF.text];
...
- (IBAction)showMyPicker:(id)sender {
    [GKHpickerAddressView shareInstancePickerAddressByctrl:self block:^(UIViewController *ctrl, NSString *addressName) {
        [sender setTitle:addressName forState:UIControlStateNormal];
        self.adds1 = [NSString stringWithFormat:@"%@",addressName];
